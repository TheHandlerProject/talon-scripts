...
fetch_verify "knox-modelfile-v3"      "FROM mistral"
...
NEW_HASH=$(md5sum "$CONTEXT_DIR/knox-modelfile-v3" 2>/dev/null | cut -d' ' -f1)
OLD_HASH=$(cat "$CONTEXT_DIR/.knox-modelfile-hash" 2>/dev/null)
if [ "$NEW_HASH" != "$OLD_HASH" ]; then
  log "Knox modelfile changed — rebuilding..."
  ollama create knox -f "$CONTEXT_DIR/knox-modelfile-v3" >> "$LOG" 2>&1 \
