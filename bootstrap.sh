...
# 1. Pull all files — using actual GitHub filenames
info "Pulling latest files from GitHub..."

declare -A FILES=(
  ["deploy-talon-1.sh"]="deploy-talon.sh"
  ["talon-drop-1.html"]="talon-drop.html"
  ["knox-modelfile-2"]="knox-modelfile"
)

for remote in "${!FILES[@]}"; do
  local="${FILES[$remote]}"
  curl -sf "$GITHUB_RAW/$remote" -o "$CONTEXT_DIR/$local" \
    && ok "$remote → $local" \
    || fail "Could not fetch $remote — check repo is public and file exists"
done

# 2. Deploy talon-drop.html to web root
info "Deploying talon-drop.html to web root..."
sudo mkdir -p /var/www/html
sudo cp "$CONTEXT_DIR/talon-drop.html" /var/www/html/talon.html
ok "talon.html live at /var/www/html"
...
ollama create knox -f "$CONTEXT_DIR/knox-modelfile" \
  && ok "Knox model built" \
  || fail "Knox model build failed — check knox-modelfile syntax"
