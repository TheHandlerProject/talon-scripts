...
FILES=("deploy-talon.sh" "talon-drop.html" "knox-modelfile" "knox-context-patch.sh")
...
# If knox-context-patch.sh changed, re-apply it
if [ -f "$CONTEXT_DIR/knox-context-patch.sh" ]; then
  PATCH_HASH=$(md5sum "$CONTEXT_DIR/knox-context-patch.sh" | cut -d' ' -f1)
  STORED_PATCH_HASH=$(cat "$CONTEXT_DIR/.knox-context-patch-hash" 2>/dev/null || echo "")
  if [ "$PATCH_HASH" != "$STORED_PATCH_HASH" ]; then
    echo "knox-context-patch.sh changed -- re-applying..."
    bash "$CONTEXT_DIR/knox-context-patch.sh" \
      && ok "claude-context patch re-applied" \
      || warn "Context patch failed -- check $CONTEXT_DIR/knox-context-patch.sh"
    echo "$PATCH_HASH" > "$CONTEXT_DIR/.knox-context-patch-hash"
  fi
fi
